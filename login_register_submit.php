...
if($type=='register'){
	...
	$captcha=get_safe_value($_POST['captcha']);
	...
	if($check>0){
		...
		mysqli_query($con,"insert into user(name,email,mobile,password,status,email_verify,added_on,rand_str,referral_code,from_referral_code) values('$name','$email','$mobile','$password','1','1','$added_on','justfordb','$referral_code','$from_referral_code')");
		...
		$arr=array('status'=>'success','msg'=>'Thank you for registering. Please Login','field'=>'form_msg');
	}
	echo json_encode($arr);
   }else{
   	  $arr=array('status'=>'success','msg'=>'Failed Login! Please enter correct captcha','field'=>'form_msg');
   	  echo json_encode($arr);
   }
}
	...
	if($check>0){	
		...
		if($email_verify==1){
			if($status==1){
				...
				mysqli_query($con,"update user set password='$rand_password' where id='$id'");
				//$html=$rand_password;
				//send_email($email,$html,'New Password');
				$arr=array('status'=>'success','msg'=>'Your new password is ' .$rand_password);
